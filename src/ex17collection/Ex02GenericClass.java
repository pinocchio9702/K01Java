...
/*
=> 기본 object기반의 FruitBox를 아래와 같이 제네릭 클래스로
변경한다. 자료형에 해당하는 붑ㄴ을 타입매개변수 T(type)로 교체하고
객체생성시 자료형을 결정하기 위해 " 클래스명<T>" 형태로 변경한다.
 */
class GenerFuritBox<T>{
	T item;
...
public class Ex02GenericClass {
	...
	public static void main(String[] args) {
		/*
		 ...
		 각각 Apple, Banan, 객체를 저장할수 있는 인스턴스가
		 생성되었다.
		 */
		GenerFuritBox<Apple> appleBox = new GenerFuritBox<Apple>();
		appleBox.store((new Apple(10)));
		Apple apple = appleBox.pullOut();
		apple.showInfo();
		
		GenerFuritBox<Banana> bananaBox = new GenerFuritBox<Banana>();
...
//		GenerFuritBox<Orange> orange = new GenerFuritBox<Orange>();
//		orange.store("나는 오랜지");
//		orange.store(apple);
//		orange.store(banana);
//		
