...
/*
 ...
 - Set의 기본적인 특성은 동일하나, Tree알고리즘을 통해 정렬되어 저장된다.
 - 정렬의 기준은 개발자가 직접 정의 할 수 있다.
...
class MyString implements Comparable<MyString>{
	...
	//맴버변수 str의 길이를 구해 반환한다.
	public int getLength() {
		return str.length();
	}
	/*
	 정렬의 기준을 위해 오버라이딩 한 메소드로 가나다와 같은 사전순, 문자열의 길이 등 다양하게
	 설정할 수 있다.
...
public class Ex04TreeSetMain {
	...
	public static void main(String[] args) {
		...
		/*
		 -문자열의 길이가 정렬의 기준일 때는 아래 객체중 7글자인 KOSMOXX는 
		 		1개만 삽입된다.
		 -문자열을 compareTo()를 통해 비교해서 정렬할 때는 모든 문자열이 
		 		Set에 삽입된다.
		 */
		tree1.add(1);
		tree1.add(4);
		tree1.add(2);
		tree1.add(3);
		System.out.println(tree1.add(2)); //Set은 중복저장이 허용안됨
		...
		tree2.add(new MyString("KOSM071"));//7
		tree2.add(new MyString("KOSM072"));//7
		tree2.add(new MyString("KOSM073"));//7
		tree2.add(new MyString("KOSM074"));//7
