...
/*
 함수형 인터페이스
 		...
 		람다식으로 구현할 추상 메소드를 선언한다. 이를 람다식을 만들기 위한 함수형 인터페이스라고 한다.
 		※ 함수형 인터페이스에는 오직 하나느이 추상 메소드만 선언할 수 있다.
 		
 @FunctionalInterface
 		: 함수형 인터페이스의 조건을 갖추었는지에 대한 검사를
 		컴파일러에게 요청하는 역할를 하는 어노테이션
 */
...
public class Ex04FunctionalInterface {
	...
	/*
	 매개변수 타입이 IKosmo4를 구현(implements)한 람다식 자체를 전달하여
