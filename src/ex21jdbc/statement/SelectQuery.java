...
/*
 ResultSet클래스
 		...
 		오라클의 자료형이
 				...
 				date타입 : getDate()
 				메소드로 각 컬럼의 테이터를 추출한다.
 		-인자는 select절의 컬럼순서대로 인데스(1부터시작)를 사용하거나
 		컬럼명르 사용할 수 있다.
 */
	@Override
	void execute() {
		try {
			...
			while(rs.next()) {
				...
				String regidate3 = rs.getString("regidate").substring(0, 13);//substring : 0번째 글짜부터 13번째까지만 잘라서 출력
				String regidate4 = rs.getString("d1"); //별칭을 그대로 사용
				
				System.out.printf("%-5s %-5s %-5s %-5s %-20s %-20s %s\n", 
