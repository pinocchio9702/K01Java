...
/*
 ...
 형식]
 		...
 		※부모클래를 상속받아 이름이 없는 자식클래스를 생성하는 개념으로 이해하면 된다.
 		상속되기 때문에 오버라이딩이 가능하게 된다. 즉, {} 안쪽은 '자식'의 영역이다.
 */
...
public class E03AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		 Student라는 이름을 가진 자식 클래스를 통해 객체를 생성하고 부모의 참조변수로 
		 참조하는 이질화 형태임
		 참조변수가 부모티입이므로 자식객체에 직접 접근하는 것은 불가능하고
		 접근시에는 반드시 다운케스팅(강제형변환)이후 접근해야 함.
		 */
		Person person = new Student("문사람", "2016");
		 ...
		 /*
		  		부모타입의 참조변수로 자식객체에 확장한 맴버에 접근할 수 없으므로 에러가 발생된다.
		  		접근하기 위해서는 다운캐스팅이 필요하지만 익명으로 타운캐스팅이 불가능하다.
		  */
...
//		 ((Student)anonyTest).jumin = "181105";
		 ...
		 //추상클래스를 상속받은 익명 클래스
		 AbstrackClass ac = new AbstrackClass() {
