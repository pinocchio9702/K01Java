...
public class E01WrapperClass2 {
	...
	public static void main(String[] args) {
		...
		/*
		 문자열을 숫자로 변경할때 반드시 숫자형식만 와야한다.
		 "원"은 숫자가 아니므로 예외가 발생된다. NumberFormatException 예외가 발생된다.
		 */
		String money = "120원";
		//System.out.println("120원 : " + Integer.parseInt(money) ); //예외발생
		
		/*
		 실수형태의 문자열을 실수로 변경할 때는 parseInt()는 사용할 수 없고 parseFloat()를 사용해야한다.
		 */
		...
		/*
		 5] isWhitespace() : 공백문자인지 판단한여 true를 반환한다.
		 */
		...
		/*
		 시나리오] 주어진 문자열안에 몆개의 공백문자(스페이스)가 있는지 판단하는 프로그램을 작성하시오
		 			참고) toChangeArray() : 문자열을 char형 배열로 반환해주는  String클래스에 정의된 메소드.
		 */
		...
		char[] whiteStringChar = whiteString.toCharArray();
		//배열의 크기만큼 반복한다.
		for(int i = 0; i < whiteStringChar.length; i++) {
			//각 index의 문자가 공백인지 검사해서...
			if(Character.isWhitespace(whiteStringChar[i])) {
				//공백문자이면 +1 증가한다.
				whiteCount++;
			}
		}
